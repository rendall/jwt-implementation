...
const getAuthHeader = (user:string = randomString(), password:string= randomString()) => {
...
describe("auth API endpoint", () => {
  ...
  const getAuth = () => new Promise<http.IncomingMessage>((resolve, reject) => {
    http.get(AUTH_ENDPOINT, (res) => resolve(res))
  });

  const sendAuth = (u:any, p:any) => new Promise<http.IncomingMessage>((resolve, reject) => {
  ...
  test(`auth endpoint is served at ${AUTH_ENDPOINT}`, (done) => getAuth().then(() => done()))
  test(`auth endpoint serves www-authenticate header`, (done) => getAuth().then((res:http.IncomingMessage) => { 
    expect(res.headers["www-authenticate"]).toBe('Basic realm="Access to restricted content", charset="UTF-8"')
    done() } ))
  test('auth endpoint without Authorization returns 401', (done) => getAuth().then((response) => {
